...
fn test_h2o_eval_xc_serial() {
    ...
    println!("LDA rho xc_eff time: {:?}", time.elapsed());
    let fps = [
    ...
    println!("GGA sigma xc_eff time: {:?}", time.elapsed());
    let fps = [
        fp((&xc_eff[0] * &weights).view()),
        fp((&xc_eff[1] * &weights).view()),
    ...
    println!("meta-GGA tau xc_eff time: {:?}", time.elapsed());
    let fps = [
        fp((&xc_eff[0] * &weights).view()),
        fp((&xc_eff[1] * &weights).view()),
