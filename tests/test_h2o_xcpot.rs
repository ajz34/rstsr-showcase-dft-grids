...
mod test_xcpot_from_dm_naive {
    ...
    fn test_rho(h2o: &H2OMolecule, perturbed_dm: &H2OPerturbedDM) {
        ...
        let mo_occ = h2o.mo_occ.view();
        let mo_coeff = h2o.mo_coeff.view();
    ...
    fn test_tau(h2o: &H2OMolecule, perturbed_dm: &H2OPerturbedDM) {
        ...
        let dm1p_ket = dm1_list.iter().map(|dm1| dm1 % &bra).collect_vec();
        let dm1p = dm1p_ket
            .iter()
            .map(|dm1p_ket| {
                let d = &bra % dm1p_ket.t();
...
mod test_xcpot_pure {
    ...
    use rstsr_showcase_dft_grids::numint_matmul::pure_xcpot::{
        rks_fxc_pot_with_eff_bra_trans_with_output, rks_fxc_pot_with_eff_with_output, rks_kxc_pot_with_eff_with_output,
    ...
    fn test_rks_fxc_pot_naive_vs_optimized(h2o: &H2OMolecule, perturbed_dm: &H2OPerturbedDM) {
        ...
        for den_type in [RHO, SIGMA, TAU] {
            ...
            rks_fxc_pot_with_eff_with_output_naive(
                den_type,
                xc_eff[2].view(),
                rho1.view(),
                ao.view(),
    ...
    fn test_rks_kxc_pot_naive_vs_optimized(h2o: &H2OMolecule, perturbed_dm: &H2OPerturbedDM) {
        ...
        for den_type in [RHO, SIGMA, TAU] {
            ...
            rks_kxc_pot_with_eff_with_output_naive(
                den_type,
                xc_eff[3].view(),
                rho1.view(),
                rho2.view(),
